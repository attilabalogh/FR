...
/*                                                                             */
/*  Notes        : The calculation steps are described in Balogh "Financial    */
...
proc sql;
    create table A_Age_02 as
        ...
        from A_Age_01_01 as a left join compm.company as b
        on a.gvkey = b.gvkey;
quit;
...
proc sql;
    create table A_Age_03 as
        select *
        from A_Age_02 as a left join crspa.dsfhdr (keep=PERMNO PERMCO BEGDAT ENDDAT HSICIG HSICMG) as b
...
/*	Regress Age on size and industry dummies */
/*	Obtain residual */
